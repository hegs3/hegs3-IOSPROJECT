...
class DBPick {
  ...
  public function pick($picid, $title, $updateD, $URL, $photosize, $auth) {
      ...
      $sql = 'INSERT INTO
                '.$tableID.'(picid, title, updateD, URL, photosize, auth)
              VALUES
              (?, ?, ?, ?, ?, ?)';

      $stmt = $this->conn->prepare($sql);
      $stmt->bind_param("sssssi", $picid, $title,$updateD, $URL, $photosize, $auth);

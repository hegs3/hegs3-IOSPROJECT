...
//            "text": "horse",
    ...
    static func photoFromJSONObject(json:[String: AnyObject]) -> Photo? {
        guard let photoID = json["id"] as? String,
            ...
            let height = json["height_h"] as? Any,
            let width = json["width_h"] as? Any,
            let url = URL(string: photoURLString),
            let dateTaken = dateFormatter.date(from: dateString) else {
                return nil
        }
        return Photo(title: title, photoID: photoID, remoteURL: url, cntDate: dateTaken, width: width, height: height, photoSize: "", auth: 0 )
    }
